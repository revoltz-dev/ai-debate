...
async function request(path, { method = 'GET', body, id } = {}) {
  const response = await fetch(scoped(path, id), { method, headers: { 'X-Admin': adminKey, 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body), cache: 'no-store' });
...
const isRunning = d => !d.presence.waiting && d.agents.length >= 2 && (d.state !== 'closed' || d.implementation?.status === 'pending' || d.implementation?.status === 'ready' && d.implementation.reviewAfter);
...
function Badge({ debate }) {
  ...
  const label = debate.presence.waiting ? t.waitingForAgents : debate.phase === 'review' ? t.reviewPhase : debate.implementation?.status === 'pending' ? t.implementationChoice : debate.implementation?.status === 'ready' ? t.implementation : t[{ closed: 'closedState', agreed: 'agreedState', open: 'openState', draft: 'draft' }[state]];
...
function Home({ debates, online, open, create, rename, remove, settings, busy, loaded }) {
  ...
  return <main className="home page">
    ...
    {!loaded ? <div className="empty"><LoaderCircle className="spin"/><p>{t.loading}</p></div> : visible.length ? <div className="debate-grid">{visible.map((d, index) => <article className={`debate-card ${isRunning(d) ? 'is-running' : ''} ${d.presence.waiting ? 'is-waiting' : ''}`} style={{ '--card-accent': agentColor(d.agents.find(a => a.id === (d.turn || d.implementation?.agentId)) || { kind: 'gemini' }) }} key={d.id}>
      ...
      <div className="card-activity"><span className="card-activity-label">{d.presence.waiting ? t.presence : isRunning(d) ? t.cardActivity : d.state === 'closed' ? t.closedState : t.draft}</span><div className="card-activity-row">{d.presence.waiting ? <><Users size={16}/><span>{d.presence.confirmed}/{d.presence.total} {t.presenceCount}</span></> : d.turn && d.state !== 'closed' ? <><Identity id={d.turn} agents={d.agents} size={19}/><span className="card-activity-time">{t.elapsed} <Elapsed since={d.turnStartedAt}/></span></> : d.implementation?.status === 'ready' && d.implementation.reviewAfter ? <><Identity id={d.implementation.agentId} agents={d.agents} size={19}/><span className="card-activity-note">{t.reviewWaiting}</span></> : d.implementation?.status === 'pending' ? <><Terminal size={16}/><span>{t.awaitingImplementer}</span></> : d.final ? <><CircleCheck size={16}/><span>{t.writtenBy}</span><Identity id={d.final.writer} agents={d.agents} size={19}/></> : <><MessageSquare size={16}/><span>{d.agents.length < 2 ? t.draft : t.noMessages}</span></>}</div></div>
...
function Implementation({ debate: d, mutate, run, busy, saveSettings, final = false }) {
  ...
  const autosave = useAutosave({
    ...
    value: { mode: config.mode, agentId: config.mode === 'agent' ? config.agentId || firstAgent : null, unanimousOnly: config.unanimousOnly, reviewAfter: config.reviewAfter },
  ...
  if (final && ready) return <div className="implementation-panel"><h3>{config.status === 'completed' ? t.reviewComplete : t.implementation}</h3><p className="row wrap"><CircleCheck size={17}/>{t.implementationAssigned} <Identity id={config.agentId} agents={d.agents} size={23}/></p><p className="muted">{config.reviewAfter && config.status === 'ready' ? t.reviewWaitingHelp : config.status === 'completed' ? t.closedHelp : t.implementationAssignedHelp}</p></div>;
  return <div className={final ? 'implementation-panel' : ''}>{final && <h3>{t.implementation}</h3>}<p className="muted">{final ? config.status === 'pending' ? t.awaitingImplementer : !adopted ? t.implementationNoSolution : config.unanimousOnly && d.forced ? t.implementationNoConsensus : t.implementationSkipped : t.implementationHelp}</p>
    ...
    {final && adopted && <><div className="row wrap"><span className="muted">{t.implementer}</span><Recipient agents={d.agents} value={manualAgentId} setValue={value => { if (value) setManualSelection({ debateId: d.id, id: value }); }} onlyAgents disabled={busy || ready}/></div><Button small tone="primary" icon={ArrowRight} disabled={busy || !permitted || !manualAgentId} onClick={() => run(() => mutate('/admin/implement', { agentId: manualAgentId }, 'POST', d.id))}>{t.implementNow}</Button>{config.status === 'pending' && <p className="muted">{t.implementationManualHelp}</p>}</>}
    </div></div>;
}
...
function AgentRow({ agent: a, debate: d, copyPrompt, busy }) {
  ...
  return <div ref={ref} data-agent-id={a.id} className={`side-agent ${active ? 'has-turn' : ''}`} style={{ '--agent-accent': agentColor(a) }} onKeyDown={e => { if (e.key === 'Escape') { setExpanded(false); ref.current.querySelector('.agent-name-toggle').focus(); } }}>
...
function Sidebar({ debate: d, home, copyPrompt, mutate, run, ask, busy, create, settings, saveSettings }) {
  ...
  const minimumRounds = d.state === 'open' && d.count > 0 ? Math.max(2, d.round) : 2;
  const rounds = useAutosave({
    key: `rounds:${d.id}`,
    value: String(d.maxRounds),
    onSave: value => saveSettings('/admin/settings', { maxRounds: Number(value) }, d.id),
    delay: 500,
    enabled: d.state !== 'closed',
    validate: value => !value.trim() || !Number.isInteger(Number(value)) || Number(value) < 2 || Number(value) > 50 ? t.invalidRounds : Number(value) < minimumRounds ? `${t.maxRounds}: ${minimumRounds}–50.` : ''
  });
  const roundsErrorId = `rounds-error-${d.id}`;
  return <div className="sidebar-content"><div className="sidebar-brand"><AppBrand name={t.appName} compact/><div><strong>{t.appName}</strong><span>{t.synced}</span></div></div><div className="sidebar-navigation"><Button small icon={ArrowLeft} onClick={home}>{t.history}</Button></div>
    ...
    <details className="side-detail" open><summary><Settings2 size={16}/>{t.controls}<ChevronDown size={14}/></summary><div className="detail-body controls"><div className="round-control"><label htmlFor="maxRounds">{t.maxRounds}</label><input id="maxRounds" type="number" min={minimumRounds} max="50" step="1" value={rounds.value} onChange={e => rounds.setValue(e.target.value)} disabled={d.state === 'closed'} aria-invalid={Boolean(rounds.error)} aria-describedby={rounds.error ? roundsErrorId : undefined}/></div>{rounds.error && <p id={roundsErrorId} className="autosave-error" role="alert">{rounds.error === t.invalidRounds || rounds.error.startsWith(`${t.maxRounds}:`) ? rounds.error : t.requestError}</p>}
      <p className="muted round-help">{t.roundsHelp}</p><Button small icon={SkipForward} disabled={busy || d.state === 'closed' || d.agents.length < 2} onClick={() => run(() => mutate('/admin/skip'))}>{t.skip}</Button>
      <Button small icon={Gavel} disabled={busy || d.state !== 'open' || !d.proposal} onClick={() => run(async () => { if (await ask({ title: t.decide, body: t.decideBody })) await mutate('/admin/decide'); })}>{t.decide}</Button>
...
function MessageComposer({ debate: d, hint, setHint, recipient, setRecipient, busy, onSend, awaitingReport }) {
  const { t, lang } = useLanguage();
  ...
  return <footer className="composer" ref={container}>{d.state === 'closed' && <div className="closed-note" role="status"><CircleCheck size={17}/>{awaitingReport ? t.reviewWaiting : d.implementation.status === 'pending' ? t.awaitingImplementer : d.phase === 'implementation' ? t.implementation : t.closedHelp}</div>}<form onSubmit={event => { event.preventDefault(); send(); }}>
...
function Live({ debate: d, home, copyPrompt, copy, mutate, run, ask, busy, create, settings, rename, online, sound, toggleSound, saveSettings }) {
  ...
  const turn = d.agents.find(a => a.id === d.turn);
  const awaitingReport = d.implementation.reviewAfter && d.implementation.status === 'ready';
  const workingAgent = turn || (awaitingReport ? d.agents.find(a => a.id === d.implementation.agentId) : null);
  ...
  return <div className="live-layout"><aside className={`sidebar ${drawer ? 'drawer-open' : ''}`}><Sidebar {...sideProps}/>{drawer && <IconButton label={t.close} icon={X} onClick={() => setDrawer(false)}/>}</aside>{drawer && <button className="drawer-backdrop" aria-label={t.close} onClick={() => setDrawer(false)}/>}
    <main className="live-main"><header className="live-header"><div className="row between live-title"><div className="row grow"><span className="mobile-only"><IconButton label={t.agents} icon={PanelLeft} onClick={() => setDrawer(true)}/></span><h1><AgentMentions text={d.title} agents={d.agents}/></h1><IconButton label={t.rename} icon={Pencil} disabled={busy} onClick={() => rename(d)}/></div><div className="row"><Connection online={online}/><IconButton label={sound ? t.soundOn : t.soundOff} icon={sound ? Volume2 : VolumeX} onClick={toggleSound}/><IconButton label={t.settings} icon={Settings2} onClick={settings}/></div></div><StatusBar debate={d}/></header>
      ...
      {workingAgent && (d.state !== 'closed' || awaitingReport) && <div className="thinking-card" style={{ '--agent-accent': agentColor(workingAgent) }}><ThinkingOrb agent={workingAgent} label={`${workingAgent.name}: ${awaitingReport ? t.reviewWaiting : t.thinking}`}/><div><div className="row wrap"><AgentIdentity agent={workingAgent} size={21}/><span className="thinking-dots"><i/><i/><i/></span></div><p>{awaitingReport ? t.reviewWaiting : d.state === 'agreed' ? t.writing : t.thinking}</p><span className="thinking-time"><Clock3 size={12}/><Elapsed since={awaitingReport ? d.implementation.assignedAt : d.turnStartedAt}/></span></div></div>}
      ...
      <MessageComposer debate={d} hint={hint} setHint={setHint} recipient={recipient} setRecipient={setRecipient} busy={busy} onSend={submit} awaitingReport={awaitingReport}/>
    </main>
  </div>;
}
...
function App() {
  ...
  const errorMessage = error => {
    ...
    const known = { bad_admin: tr.badAdmin, no_such_debate: tr.notFound, maxRounds_2_50: tr.invalidRounds, maxRounds_below_current_round: lang === 'pt' ? 'O limite não pode ser menor que a rodada atual.' : 'The limit cannot be lower than the current round.', debate_started: lang === 'pt' ? 'O debate já começou; os participantes não podem ser alterados.' : 'The debate has started; participants cannot be changed.' };
  ...
  const sync = useCallback(async () => {
    ...
    try {
      do {
        ...
        dataRef.current = next; setStates(next); setDebates(list); setOnline(true); setLoaded(true);
      } while (syncAgain.current && !dead.current);
    } catch (error) { if (!dead.current) { setOnline(false); if (error.code === 'bad_admin') notify(languageRef.current.t.badAdmin, true); } }
  ...
  useEffect(() => {
    ...
    sync();
    const events = new EventSource(`/events?key=${encodeURIComponent(adminKey)}`);
    const change = () => sync();
